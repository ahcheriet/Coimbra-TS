...
def fit_MM(rankings, s0=None): #returns sigma, phi
    ...
    try:
        ...
        raise
    # theta = - np.log(phi)
    return s0, theta #np.exp(-theta)#=phi
...
def samplingMM(m,n,theta=None, phi=None, k=None):
    ...
    if k==n:k=None
    if theta == 0:
        return np.array([np.random.permutation(range(n)) for i in range(m)])
...
import queue
def fitness_fsp(sol, cost):
    return makespan(cost, sol)
#    print(sol)
    n,m = cost.shape # num jobsand m=machines
...
def read_file(fil):
    ...
    with open(fil) as fp:
        for line in fp:
            #print(line, line.startswith('EOF'),len(line.strip()))
            if read and not line.startswith('EOF') and len(line.strip())>0:
                dat.append([int( x )for x in line.split()])
